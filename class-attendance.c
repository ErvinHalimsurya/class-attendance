...
void datamenu();
void mainmenu();
void insertStudent(char nama[],char nisn[], char alamat[],char telp[],int noAbsen);
void deleteStudent();
void printKelas();
void absensi();
void insertAbsen();
...
void datamenu(){
    ...
    char nisn[NISN_SIZE],telp[TELP_SIZE];
    for (;;) {
        printf(" ------------------------------------------------------------\n");
        ...
        printf(" [5] - Urutkan Nomor Absen murid berdasarkan NISN\n");
        ...
        if (chc == 1) {
            ...
            while(1){ //input nisn dengan format yang benar
            ...
            count++;
            int noAbsen=count;
            insertStudent(nama,nisn,alamat,telp,noAbsen);
            system("CLS");
        }
        else if (chc == 2) {
        	deleteStudent();
...
void deleteStudent(){
    ...
    //Jika linked list ada isi minta input berdasarkan pilihan nama atau nisn
    for(;;){
        printf("\nBerdasarkan :\n[1]-Nama \n[2]-NISN \n");
        ...
        //jika mau delete berdasarkan nisn
        else if(chc==2){

            printf("\nMasukkan NISN murid yang ingin dihapus\n:");
            gets(NISN);
        ...
        //jika nama atau nisn yang ingin dicari ditemukan
        if(found==1){
            count--;
...
void printKelas() {
    ...
    printf(" ---------------------------------------\n");
    printf("List : \n");
        printf("No. Absen|              Nama              |       NISN      |       No. Telp       |        Alamat\n");
        printf("---------|--------------------------------|-----------------|----------------------|----------------------\n");
    while (t->nextStudent != NULL) {
        printf("   %-3d   | %-30s | %-15s | %-20s | %-100s\n",t->noAbsen, t->nama,t->nisn,t->telp,t->alamat);
        t = t->nextStudent; 
        j++;
    }

    printf("   %-3d   | %-30s | %-15s | %-20s | %-100s\n",t->noAbsen, t->nama,t->nisn,t->telp,t->alamat); // Print last Class
}
...
void insertAbsen(char targetName[]){
    ...
    if(found==1){
        ...
        strcpy(varStorage->nisn,tmpClass->nisn);
        ...
        if (tm.tm_wday==0){
        	strcpy(varStorage->hari,"Minggu");
		}
		else if(tm.tm_wday==1){
			strcpy(varStorage->hari,"Senin");
		}
		else if(tm.tm_wday==2){
			strcpy(varStorage->hari,"Selasa");
		}
		else if(tm.tm_wday==3){
			strcpy(varStorage->hari,"Rabu");
		}
		else if(tm.tm_wday==4){
			strcpy(varStorage->hari,"Kamis");
		}
		else if(tm.tm_wday==5){
			strcpy(varStorage->hari,"Jumat");
		}
		else if(tm.tm_wday==6){
			strcpy(varStorage->hari,"Sabtu");
		}
...
void printAbsen(){
    struct Absen *t;
        ...
        printf("  No. |   No. Absen  |              Nama               |           NISN         |        Waktu Absen\n");
        printf("------|--------------|---------------------------------|------------------------|---------------------------\n");
        while (t->nextStudent != NULL) {
            printf("   %-3d|      %-5d   | \t%-30s |\t%-15s |\t%d:%d %s %d-%d-%d\n",j,t->noAbsen, t->nama,t->nisn,t->jam,t->menit,t->hari,t->tanggal,t->bulan+1,t->tahun+1900);
            t = t->nextStudent;
            j++;
        } //Tahun ditambah 1900 dan bulan ditambah 1 karena time.h mulai menghitung tahun sejak 1900 dan bulan pada time.h dihitung dari 0-11
        printf("   %-3d|      %-5d   | \t%-30s |\t%-15s |\t%d:%d %s %d-%d-%d\n",j,t->noAbsen, t->nama,t->nisn,t->jam,t->menit,t->hari,t->tanggal,t->bulan+1,t->tahun+1900); // Print last absen
}
void sortNama(){
    struct Student *t;
...
void sortNISN(){
    ...
    do{
        ...
        t=class;
        while(t->nextStudent!=temp){
            if (strcmp(t->nisn, t->nextStudent->nisn)>0){
                swap(t,t->nextStudent);
                swapped=1;
            }
            t=t->nextStudent;
...
void swap(struct Student *a,struct Student *b){
    ...
    //swap nisn
    strcpy(tempNISN,a->nisn);
    strcpy(a->nisn,b->nisn);
    strcpy(b->nisn,tempNISN);
...
void searchStudent(){
    ...
    for(;;){
        ...
        //jika mau mencari berdasarkan nisn
        else if(chc==2){

            printf("\nMasukkan NISN murid yang ingin dicari\n:");
            gets(NISN);
        ...
        if(found==1){
        	printf("\n");
        	printf("   No. Absen   |          Nama             |        NISN       |              Alamat              |    No. Telp\n");
            printf(" \t%-3d    |      %-20s |   %-15s |   %-30s |   %-15s \n",t->noAbsen, t->nama,t->nisn,t->alamat,t->telp);
            break;
        }   
        //jika data yang dicari tidak ditemukan
        else {
...
void castToFile(){
    ...
    t = absen;
    fprintf(f," --------------------------------\n");
    fprintf(f,"| Murid yang Hadir pada %s %d/%d/%d |\n",t->hari,t->tanggal,t->bulan+1,t->tahun+1900); //Bulan di +1 karena time.h menhitung bulan dari 0-11 dan tahun di +1900 karen time.h mulai merekam tahun dari tahun 1900
    fprintf(f," --------------------------------\n");
    fprintf(f,"  No. |   No. Absen  |              Nama               |           NISN         |        Waktu Absen\n");
    fprintf(f,"------|--------------|---------------------------------|------------------------|---------------------------\n");
    while (t->nextStudent != NULL) {
        fprintf(f,"   %-3d|      %-5d   | \t%-30s |\t%-15s |\t%d:%d %s %d-%d-%d\n",j,t->noAbsen, t->nama,t->nisn,t->jam,t->menit,t->hari,t->tanggal,t->bulan+1,t->tahun+1900);
        t = t->nextStudent;
        j++;
    }
    fprintf(f,"   %-3d|      %-5d   | \t%-30s |\t%-15s |\t%d:%d %s %d-%d-%d\n",j,t->noAbsen, t->nama,t->nisn,t->jam,t->menit,t->hari,t->tanggal,t->bulan+1,t->tahun+1900); // Print last absen
    ...
    fprintf(f," -------------------------------\n");
    fprintf(f,"  No. |   No. Absen  |              Nama               |           NISN          \n");
    fprintf(f,"------|--------------|---------------------------------|------------------------\n");
    while (tempClass!=NULL){ 
        ...
        if(found==0){ //jika data ditemukan maka tidak lakukan apa-apa, tetapi jika tidak ditemukan, print data murid saat ini
            fprintf(f,"   %-3d|      %-5d   | \t%-30s |\t%-15s \n",j,tempClass->noAbsen, tempClass->nama,tempClass->nisn);
            j++;
        }
...
void saveToFile(){
    ...
    while(tmp!=NULL){
        ...
        fwrite((tmp->nisn), sizeof (tmp->nisn), 1, data); 
...
void readFromFile(){
    ...
    if (data == NULL){
        ...
        while(1){
            ...
            if (rlen != 1)
                break;
            fread((a->nisn), sizeof a->nisn, 1, data);
            if (rlen != 1)
                break;
            ...
            count++;
            insertStudent(a->nama,a->nisn,a->alamat,a->telp,a->noAbsen);
        }
        fclose(data);
    }
